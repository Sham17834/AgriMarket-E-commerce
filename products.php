...
class Product
{
    ...
    public function getProducts($search = '', $category = 0, $organic = null, $min_price = 0, $max_price = 1000, $sort_by = 'name', $sort_order = 'ASC', $page = 1, $products_per_page = 12)
    {
        ...
        try {
            ...
            foreach ($params as $key => $value) {
                ...
                } elseif ($key === 'category' || $key === 'organic' || $key === 'limit' || $key === 'offset') {
            ...
            error_log("Fetched " . count($results) . " products for page $page");
            return $results;
    ...
    public function getProductCount($search = '', $category = 0, $organic = null, $min_price = 0, $max_price = 1000)
    {
        ...
        try {
            ...
            foreach ($params as $key => $value) {
                $paramKey = ':' . $key;
                if ($key === 'search' || $key === 'min_price' || $key === 'max_price') {
                    $stmt->bindValue($paramKey, $value, PDO::PARAM_STR);
                } else {
                    $stmt->bindValue($paramKey, $value, PDO::PARAM_INT);
                }
            ...
            error_log("Total products counted: $count");
            return $count;
        } catch (PDOException $e) {
    ...
    public function getCategories()
    {
        try {
            ...
            error_log("Fetched " . count($categories) . " categories: " . json_encode($categories));
            return $categories;
        } catch (PDOException $e) {
...
$products = $product->getProducts($search, $category, $organic, $min_price, $max_price, $sort_by, $sort_order, $page, $products_per_page);
$total_products = $product->getProductCount($search, $category, $organic, $min_price, $max_price);
...
<body class="bg-natural-light min-h-screen font-body">
        ...
        <section class="py-16 bg-white">
            <div class="max-w-7xl mx-auto px-4">
                ...
                <div class="flex flex-wrap lg:flex-nowrap">
                    <!-- Sidebar filters -->
                    <div class="sidebar-filter w-full lg:w-64 bg-white p-6 rounded-lg shadow-sm mr-0 lg:mr-8" id="sidebarFilter">
                        ...
                        <form method="GET" id="filterForm">
                                ...
                                <div>
                                    <div class="space-y-2 mt-3">
                                        <div class="flex items-center">
                                            <input type="radio" id="cat-all" name="category" value="0" class="mr-2" <?php if ($category == 0) echo 'checked'; ?>>
                                            <label for="cat-all" class="<?php if ($category == 0) echo 'active-filter'; ?>">All Categories</label>
                                        </div>
                                        <?php if (!empty($categories) && is_array($categories)): ?>
                                            <?php foreach ($categories as $cat): ?>
                                                <div class="flex items-center">
                                                    <input type="radio" id="cat-<?php echo $cat['category_id']; ?>" name="category" value="<?php echo $cat['category_id']; ?>" class="mr-2" <?php if ($cat['category_id'] == $category) echo 'checked'; ?>>
                                                    <label for="cat-<?php echo $cat['category_id']; ?>" class="<?php if ($cat['category_id'] == $category) echo 'active-filter'; ?>">
                            ...
                            <div class="filter-section">
                                ...
                                <div>
                                    <div class="space-y-2 mt-3">
                                        <div class="flex items-center">
                                            <input type="radio" id="organic-all" name="organic" value="" class="mr-2" <?php if ($organic === null) echo 'checked'; ?>>
                                            <label for="organic-all" class="<?php if ($organic === null) echo 'active-filter'; ?>">All Products</label>
                                        </div>
                                        <div class="flex items-center">
                                            <input type="radio" id="organic-yes" name="organic" value="1" class="mr-2" <?php if ($organic === 1) echo 'checked'; ?>>
                                            <label for="organic-yes" class="<?php if ($organic === 1) echo 'active-filter'; ?>">Organic Only</label>
                                        </div>
                                        <div class="flex items-center">
                                            <input type="radio" id="organic-no" name="organic" value="0" class="mr-2" <?php if ($organic === 0) echo 'checked'; ?>>
                                            <label for="organic-no" class="<?php if ($organic === 0) echo 'active-filter'; ?>">Standard Only</label>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="filter-section">
                                ...
                                <div>
                                    ...
                                    <div class="flex justify-between mt-2">
                                        <div>
                                            <span>RM</span>
                                            <input type="number" name="min_price" id="min_price" value="<?php echo $min_price; ?>" class="border border-gray-200 rounded w-16 text-sm px-2 py-1">
                                        </div>
                                        <div>
                                            <span>RM</span>
                                            <input type="number" name="max_price" id="max_price" value="<?php echo $max_price; ?>" class="border border-gray-200 rounded w-16 text-sm px-2 py-1">
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="filter-section">
                                ...
                                <div>
                                    <select name="sort_by" class="w-full border border-gray-200 rounded p-2 text-sm">
                                        ...
                                        <option value="business_name" <?php if ($sort_by == 'business_name') echo 'selected'; ?>>Vendor</option>
                                    </select>
                                    <div class="flex mt-3">
                                        <div class="flex items-center mr-4">
                                            <input type="radio" id="sort-asc" name="sort_order" value="ASC" class="mr-2" <?php if ($sort_order == 'ASC') echo 'checked'; ?>>
                                            <label for="sort-asc">Ascending</label>
                                        </div>
                                        <div class="flex items-center">
                                            <input type="radio" id="sort-desc" name="sort_order" value="DESC" class="mr-2" <?php if ($sort_order == 'DESC') echo 'checked'; ?>>
                            ...
                            <div class="mt-6 space-y-3">
                                <button type="submit" class="w-full bg-primary text-white px-4 py-2 rounded-full hover:bg-primary-dark transition-colors">
                                    Apply Filters
                                </button>
                            </div>
                        </form>
                    ...
                    <div class="w-full">
                        ...
                        <div class="flex flex-wrap items-center justify-between mb-6 p-4 bg-gray-50 rounded-lg">
                            <div>
                                <p class="text-sm text-gray-600">Showing <span class="font-semibold"><?php echo $product_count; ?></span> of <span class="font-semibold"><?php echo $total_products; ?></span> products</p>
                                <?php if ($search): ?>
                                    <p class="text-sm">Search results for: <span class="font-semibold">"<?php echo htmlspecialchars($search); ?>"</span></p>
                                <?php endif; ?>
                            </div>
                        </div>
                        ...
                        <div class="product-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            <?php if (!empty($products)): ?>
                                <?php foreach ($products as $row): ?>
                                    <div class="product-card bg-white rounded-lg shadow-sm overflow-hidden hover:shadow-md transition-all">
                                        <div class="relative overflow-hidden group">
                                            ...
                                            if (json_last_error() !== JSON_ERROR_NONE) {
                                                error_log("JSON decode error for product_id {$row['product_id']}: " . json_last_error_msg());
                                                $image_url = 'https://public.readdy.ai/api/placeholder/400/320';
                                            } else {
                                                $image_url = is_array($image_urls) && !empty($image_urls) ? $image_urls[0] : 'https://public.readdy.ai/api/placeholder/400/320';
                                            }
                                            ?>
                                            <a href="product_details.php?id=<?php echo $row['product_id']; ?>">
                                                <img src="<?php echo htmlspecialchars($image_url); ?>" alt="<?php echo htmlspecialchars($row['name']); ?>" class="w-full h-48 object-cover transition-transform duration-500 group-hover:scale-110">
                                            </a>
                                            <div class="absolute inset-0 bg-black bg-opacity-20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center pointer-events-none"></div>
                                        </div>
                                        <div class="p-4">
                                            <div class="flex items-center justify-between mb-2">
                                                <span class="text-sm text-gray-500"><?php echo htmlspecialchars($row['category_name']); ?></span>
                                            </div>
                                            ...
                                            <div class="flex items-center mt-2">
                                                <?php if (!is_null($row['discounted_price']) && $row['discounted_price'] < $row['price']): ?>
                                                    <span class="text-gray-500 line-through text-sm mr-2">RM<?php echo number_format($row['price'], 2); ?></span>
                            ...
                            <?php else: ?>
                                <div class="col-span-3 text-center py-16 bg-gray-50 rounded-lg">
                                    ...
                                    <a href="products.php" class="inline-block mt-4 text-primary hover:underline">View all products</a>
                                </div>
                            <?php endif; ?>
                        </div>
                        ...
                        <?php if ($total_pages > 1): ?>
                            <div class="pagination">
                                ...
                                $query_params = array_filter([
                                    ...
                                    'organic' => $organic !== null ? $organic : null,
                                    'min_price' => $min_price != $db_min_price ? $min_price : null,
                                    'max_price' => $max_price != $db_max_price ? $max_price : null,
                                    'sort_by' => $sort_by != 'name' ? $sort_by : null,
                                    'sort_order' => $sort_order != 'ASC' ? $sort_order : null,
                                ]);
                                $base_url = 'products.php?' . http_build_query($query_params);

                                // Previous button
                                if ($page > 1) {
                                    $prev_url = $base_url . ($query_params ? '&' : '') . 'page=' . ($page - 1);
                                    echo '<a href="' . htmlspecialchars($prev_url) . '" class="pagination-prev">Previous</a>';
                                } else {
                                    echo '<span class="pagination-prev disabled">Previous</span>';
                                }
                                ...
                                if ($start_page > 1) {
                                    echo '<a href="' . htmlspecialchars($base_url . ($query_params ? '&' : '') . 'page=1') . '">1</a>';
                                    if ($start_page > 2) {
                                        echo '<span>...</span>';
                                    }
                                }
                                ...
                                for ($i = $start_page; $i <= $end_page; $i++) {
                                    $page_url = $base_url . ($query_params ? '&' : '') . 'page=' . $i;
                                    $active_class = $i == $page ? 'active' : '';
                                    echo '<a href="' . htmlspecialchars($page_url) . '" class="' . $active_class . '" aria-current="' . ($i == $page ? 'page' : 'false') . '">' . $i . '</a>';
                                }
                                ...
                                if ($end_page < $total_pages) {
                                    ...
                                    echo '<a href="' . htmlspecialchars($base_url . ($query_params ? '&' : '') . 'page=' . $total_pages) . '">' . $total_pages . '</a>';
                                }

                                // Next button
                                if ($page < $total_pages) {
                                    $next_url = $base_url . ($query_params ? '&' : '') . 'page=' . ($page + 1);
                                    echo '<a href="' . htmlspecialchars($next_url) . '" class="pagination-next">Next</a>';
                                } else {
                                    echo '<span class="pagination-next disabled">Next</span>';
                                }
                                ?>
                            </div>
                        <?php endif; ?>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (mobileFilterToggle && sidebarFilter && sidebarOverlay && closeSidebar) {
