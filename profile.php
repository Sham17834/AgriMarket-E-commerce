...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (array_filter($password_fields)) { // If any password field is filled
            if (!$form_data['current_password']) {
                throw new Exception("Current password is required to change password.");
            }
            if (!password_verify($form_data['current_password'], $user['password_hash'])) {
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="password" id="current_password" name="current_password" value="<?php echo htmlspecialchars($form_data['current_password']); ?>" 
                                        class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent">
                                </div>
                                <div>
